...
//    			if(ser.serverType == sType && ser.state == 2)
    ...
    public Server getMyAlg() {
    	Server myServer = null;
    	if(inUseServer.size()==0)
    		myServer = getBestFit(listServer);
    	else {
    		System.out.println(inUseServer.size());
    		myServer = getBestFit(inUseServer);
    		if(myServer == null || currentJob.subTime > myServer.avbTime) {
    ...
    public void scheduleJob(PrintStream pr, Socket s, Integer useAlg) throws UnsupportedEncodingException, IOException //add parser par
    {
    	...
    	if (useAlg == null) {  //null or not ff
    		...
    		ser = getFirstFit();
    		str = getFirstFit().serverType + " " + ser.serverID;
    	}
    	else if (useAlg == 2) {
            ser = getBestFit(listServer);
        	str = getBestFit(listServer).serverType + " " + ser.serverID;;
        }
        else if (useAlg == 3) {
    	...
    	ser.avbTime = currentJob.subTime + currentJob.est + 3600;
    	addToInUse(ser);
    ...
    public static void main(String[] args) throws UnknownHostException, IOException {
    	...
    	if (args.length != 0)
    	{
            ...
            if(args[0].contains("-a") && args[1].contains("wf"))
                algorithm = 3;
            if(args[0].contains("-a") && args[1].contains("mf"))
            	algorithm =4;
